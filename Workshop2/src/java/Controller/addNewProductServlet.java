...
public class addNewProductServlet extends HttpServlet {
    ...
    private final String SUCCESS = "getAllServlet";
    private final String FAIL = "error.jsp";
    private final String ERROR = "addNewProductForm.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            else if(!ProName.trim().equals(""))
            {
            if (dao.addNewProduct(ProID, ProName, Price, image) == 1) {
                url = SUCCESS;
            }
            }
            else
            {
                HttpSession session = request.getSession();
                session.setAttribute("EMPTY", "Product Name cannot empty!!");
                 url = "addNewProductForm.jsp";
            }
